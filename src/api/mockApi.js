...
export const SEED_USERS = [
  ...
  // },
  // {
  //   id: "u4", name: "Fayzulloh Umidjonov",   phone: " ",
  //   password: "Fayzulloh",   role: "support", avatar: "FU ", color: "#7c3aed",
  //   joinDate: "01.01.2026",
  // },
  // {
...
// Web (GW)
export const WEB_GROUPS = Array.from(
  { length: 200 },
  (_, i) => `GW${String(i + 1).padStart(2, "0")}`,
);

// Robototexnika (GR01–GR40)
const ROBOT_GROUPS = Array.from(
  { length: 40 },
  (_, i) => `GR${String(i + 1).padStart(2, "0")}`,
);

// Design (D01–D20)
const DESIGN_GROUPS = Array.from(
  { length: 20 },
  (_, i) => `D${String(i + 1).padStart(2, "0")}`,
);

export const GROUPS = [...WEB_GROUPS, ...ROBOT_GROUPS, ...DESIGN_GROUPS];
...
const SEED_RECORDS = [
  ...
  },
  {
    id: "r5",
    supportId: "u4",
    supportName: "Davron Yusupov",
    date: "19.02.2026",
    time: "13:00",
    group: "GW115",
    mentor: "Nurmuhammad",
    student: "Zulfiya Qodirov",
    theme: "Node.js intro",
    status: "group",
  ...
  },
  {
    id: "r6",
    supportId: "u5",
    supportName: "Ezgulik Nazarova",
    date: "19.02.2026",
    time: "15:30",
    group: "GW116",
    mentor: "Mirodil",
    student: "Humoyun Baxtiyorov",
    theme: "Express routes",
    status: "coworking",
...
function initStorage() {
  ...
  if (!localStorage.getItem(LS_RECORDS))
    localStorage.setItem(LS_RECORDS, JSON.stringify(SEED_RECORDS));
...
function saveNotifs(n) {
  localStorage.setItem(LS_NOTIFS, JSON.stringify(n));
}
...
export async function apiRegister(data) {
  ...
  const newUser = {
    ...
    avatar: data.name
      ...
      .slice(0, 2)
      .toUpperCase(),
    color: ["#3d5eff", "#059669", "#d97706", "#7c3aed", "#db2777"][
...
export async function apiGetRecords(filter = {}) {
  ...
  if (filter.supportId)
    records = records.filter((r) => r.supportId === filter.supportId);
